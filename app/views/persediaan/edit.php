            ...
            <div class="row">
                <div class="col-md-4 mb-3">
                    ...
                    <select id="akun_persediaan" name="akun_persediaan" class="form-select" required>
                        <?php foreach($data['akun'] as $akun){ if(substr($akun['kode_akun'],0,1)=='1' && $akun['tipe_akun']!='Header'){ echo "<option value='{$akun['kode_akun']}' ".($akun['kode_akun']==$data['barang']['akun_persediaan']?'selected':'').">{$akun['nama_akun']}</option>"; } } ?>
                    </select>
                </div>
                <div class="col-md-4 mb-3">
                    <label for="akun_hpp" class="form-label">Akun HPP</label>
                    <select id="akun_hpp" name="akun_hpp" class="form-select" required>
                        <?php foreach($data['akun'] as $akun){ if(substr($akun['kode_akun'],0,1)=='5' && $akun['tipe_akun']!='Header'){ echo "<option value='{$akun['kode_akun']}' ".($akun['kode_akun']==$data['barang']['akun_hpp']?'selected':'').">{$akun['nama_akun']}</option>"; } } ?>
                    </select>
                </div>
                <div class="col-md-4 mb-3">
                    ...
                    <select id="akun_penjualan" name="akun_penjualan" class="form-select" required>
                        <?php foreach($data['akun'] as $akun){ if(substr($akun['kode_akun'],0,1)=='4' && $akun['tipe_akun']!='Header'){ echo "<option value='{$akun['kode_akun']}' ".($akun['kode_akun']==$data['barang']['akun_penjualan']?'selected':'').">{$akun['nama_akun']}</option>"; } } ?>
                    </select>
                </div>
            </div>
            <hr>
